...
static int hdd_config_ant_probe_threshold(struct hdd_adapter *adapter,
					  struct nlattr *tb[])
{
	struct nlattr *wlan_thre_attr =
		tb[ANT_DIV_PROBE_WLAN_RSSI_THRESHOLD];
	struct nlattr *bt_thre_attr =
		tb[ANT_DIV_PROBE_BT_RSSI_THRESHOLD];
	...
	ant_probe_threshold =
		ANT_DIV_SET_PROBE_THRESHOLD(wlan_threshold, bt_threshold);
						...
						struct nlattr *tb[])
{
	...
	ant_rssi_diff =
		ANT_DIV_SET_RSSI_DIFF(wlan_rssi_diff, bt_rssi_diff);
